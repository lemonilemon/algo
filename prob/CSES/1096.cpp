...
void solve() {
	...
	if(n <= 6) cout << vdp[n - 1];
		...
		reverse(vdp.begin(), vdp.end());
		Mat dp(6, 1, vdp);
		vector<int> vtrans;
		for(int i = 0; i < 6; ++i) {
			vtrans.emplace_back(1);
		}
		for(int i = 1; i < 6; ++i) {
			for(int j = 0; j < 6; ++j) {
				if(j == i - 1) vtrans.emplace_back(1);
				else vtrans.emplace_back(0);
			}
		}
		Mat trans(6, 6, vtrans);
		cout << (fastpow(trans, n - 6) * dp).val[0][0] << '\n';
	}
}
